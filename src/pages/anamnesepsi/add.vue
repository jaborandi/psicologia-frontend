...
<script setup>
	...
	const props = defineProps({
		...
		routeName : {
			type : String,
			default : 'anamnesepsiadd',
		},
		apiPath : {
			type : String,
			default : 'anamnesepsi/add',
		},
		submitButtonLabel: {
			type: String,
			default: "Enviar",
	...
	//auto fill other inputs on municipe_id change
	async function municipeIdAutofill(val){
		const resp = await ApiService.get(`components_data/anamnesepsi_municipe_id_autofill?value=${val}`);
		const data = resp.data[0];
		if(data){
formData.nome_pai = data.pai;
formData.nome_mae = data.mae;
		}
	}
